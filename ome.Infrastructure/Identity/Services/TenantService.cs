...
public class TenantService(
    ...
    public Guid GetCurrentTenantId() {
        try {
            ...
            foreach (var tenantIdPart in from @group in groups
                     where @group.StartsWith("tenant:", StringComparison.OrdinalIgnoreCase)
                     select @group.Split(':').LastOrDefault()) {
    ...
    public async Task<bool> InitializeTenantContextAsync(string companyId,
        ...
        try {
            // Normalisiere die Company ID für Vergleich
            // In deiner ursprünglichen Version war: var normalizedCompanyId = companyId.ToUpperInvariant();
            // Wir ändern das zu ToLowerInvariant() für bessere Konsistenz mit den ToLower() Aufrufen
            var normalizedCompanyId = companyId.ToLowerInvariant();

            logger.LogInformation("Suche nach Tenant mit Name oder DisplayName: {CompanyId}", companyId);

            // Hier DbContext über Factory erstellen
            await using var dbContext = await dbContextFactory.CreateDbContextAsync(cancellationToken);

            // Suche nach Tenant mit Name oder DisplayName
            // EF Core kann StringComparison.CurrentCultureIgnoreCase nicht übersetzen, daher verwenden wir ToLower()
            var tenant = await dbContext.Tenants
                .AsNoTracking()
                .IgnoreQueryFilters()
                .FirstOrDefaultAsync(t =>
            ...
            logger.LogError(ex, "Fehler bei der Tenant-Suche für {CompanyId}: {ErrorMessage}",
